...
void Student::get()
	{
		...
		cin.ignore();                                     
		getline(cin,addr);                   //is used to read line of text from input stream and store it in a string cariable
		cout<<"Enter mobile number."<<endl;        //it allowes u to read input containing spaces
		cin>>mob;
	}
